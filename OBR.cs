...
public class MultiMotors {
    ...
    public async Task stop(double time=50) {
        // inertia_time = (Math.Abs(last_speed) / 200d) * 100d;
        // await together(0, 0);
        // Lock(true);
        // await Time.Delay(50);
        // Lock(false);
        // await Time.Delay(inertia_time);
...
async Task Main() {
    ...
    if (debug_mode) {
        ...
        both.Lock(false);
        ///
        await both.turnDegree(500, 400, 90);
        await both.stop();
        IO.PrintLine($@"
            inclination: {Bot.Inclination}
            distance: {getDistance(ultraMid)}
        ");
        IO.PrintLine("stopped");
    }
    await armUp();
    await handOpen();
    both.Lock(false);
...
async Task reachWallBackwards(double minimum_distance=2.1) {
    ...
    while(getDistance(ultraBack)>Math.Abs(minimum_distance) && !had_green && !hasTouched()) {
        await both.together(baseforce, -basespeed);
        await Time.Delay(root_delay);
        had_green = hasSomeGreen() || isRescue();
    }
    await both.stop();
    if (had_green) {
...
async Task searchExit(double c=1) {
    ...
    await both.together(baseforce, basespeed, 0.1d);
    ...
    while(!found_exit) {
        ...
        if (blue_above) {
            ...
            await both.turnDegree(baseforce, turnspeed, 92*c);
            await alignDirection();
...
async Task getVictim(double c=1, double distance_backwards=2.1) {
    ...
    // go back
    await alignDirection();
    IO.PrintLine($"distance_backwards={distance_backwards}");
    await reachWallBackwards(distance_backwards);
}

bool isArenaUp(double distance=0) {
    return (distance>=25); //30
}
...
async Task RescueProcess() {
    ...
    if (fast_found) {
        ...
        is_up = isArenaUp(last_distance); // em pe
        is_down = isArenaDown(last_distance); // deitada
        await both.together(baseforce, basespeed, 2d);
        await alignDirection();
    }
    ...
    await alignDirection();
    await both.turnDegree(baseforce, turnspeed, 45*-c);
    await reachWall(ultraMidAux, minimum_distance: 4);
    await both.turnDegree(baseforce, turnspeed, 45*c);
    await alignDirection();
    bool must_deliver = false;
    double box_distance = getDistance(ultraBack);
    ...
    while(!must_deliver) {
        ...
        await both.stop();
        await Time.Delay(100);
        IO.PrintLine($"box_distance={box_distance}");
        await both.turnDegree(baseforce, turnspeed, 90*c);
        await alignDirection();
        wall_distance = getDistance(ultraBack);
        IO.PrintLine($"wall_distance={wall_distance}");
        ...
        await both.stop();
        await getVictim(c, distance_backwards: ((box_distance<10.2d) ? (wall_distance+0.1) : 2.1));
        await alignDirection();
        await both.turnDegree(baseforce, turnspeed, 90*-c);
        await alignDirection();
