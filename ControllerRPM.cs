...
public class Motor {
    ...
    public string name;
    //////////////////
    public double kp = 5; //5
    public double ki = 1.1; //1.1
    public double kd = 0.1; //0.1
    ...
    public double computePID(double input, double sp, bool must_derivate=true) {
        ...
        IO.Print($@"motor: {name}
            ...
            i: {Convert.ToInt32(i)}
            d: {Convert.ToInt32(d)}
            pid(pwm): {Convert.ToInt32(pid)} || .Velocity: {motor.Velocity}
        ");
        last_error = error;
        return pid;
    }
